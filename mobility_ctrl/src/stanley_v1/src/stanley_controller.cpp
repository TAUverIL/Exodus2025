...
double StanleyController::computeSteeringAngle(const geometry_msgs::msg::PoseStamped & robot_pose,
  ...
  RCLCPP_INFO(logger_, "Yaw Path: %.3f, Curr Yaw: %.3f, Angle Err: %.3f, X-track Err: %.3f", yaw_path_deg, curr_yaw_deg, StanleyController::radToDeg(angle_err), xtrack_err_corr_deg);
  return delta;
  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  double curr_yaw = StanleyController::radToDeg(StanleyController::getNormalizedAngle(tf2::getYaw(pose.pose.orientation)));

  RCLCPP_INFO(logger_, "CTE: %.3f Tgt Angle: %.3f, Curr Yaw: %.3f, Curr Vel: %.3f", cte, StanleyController::radToDeg(target_angle), curr_yaw, speed.linear.x);
