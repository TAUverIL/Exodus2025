  ...
  nav2_core::GoalChecker * goal_checker)
{
  ...
  // double sign = 1.0;
  // if (allow_reversing_) {
  //   auto [idx, dist] = findNearestIndex(transformed_plan, 0.0, 0.0);
  //   if (transformed_plan.poses[idx].pose.position.x < 0.0) {
  //     sign = -1.0;
  //   }
  // }
  ...
  // geometry_msgs::msg::TwistStamped cmd_vel_old;
  // cmd_vel_old.header = pose.header;
  // cmd_vel_old.twist.linear.x = linear_vel;
  // cmd_vel_old.twist.angular.z = angular_vel;
  ...
  // FIXME - check if we need to use speed or desired_linear_vel_
  target_angle = computeSteeringAngle(pose, desired_linear_vel_);
  ...
  cmd_vel.twist.angular.z = speed_update * std::tan(target_angle) / wheel_base_;               // yaw rate = v * curvature
  ...
  front_axis.header.frame_id = "base_link";          // <-- key line
  front_axis.ns   = "stanley_controller";
  front_axis.id   = 0;
  front_axis.type = visualization_msgs::msg::Marker::SPHERE;
  front_axis.action = visualization_msgs::msg::Marker::ADD;

  front_axis.pose.position.x = wheel_base_ / 2.0;  // ½ wheel-base forward
  front_axis.pose.position.y = 0.0;
  front_axis.pose.position.z = 0.1;           // or a small positive value
  // front_axis.pose.orientation.w = 1.0;        // identity rotation

  front_axis.scale.x = front_axis.scale.y = front_axis.scale.z = 0.08;  // 8-cm sphere
  front_axis.color.r = 0.8f;
  front_axis.color.g = 0.4f;
  front_axis.color.b = 0.0f;
  front_axis.color.a = 1.0f;

  marker_pub_->publish(front_axis);
