...
#include "vec_pursuit/vec_pursuit_controller.hpp"
...
namespace vec_pursuit_controller
{
void VecPursuitController::configure(
...
void VecPursuitController::cleanup()
{
  RCLCPP_INFO(
    logger_,
...
void VecPursuitController::activate()
{
  RCLCPP_INFO(
    logger_,
    ...
    std::bind(
      &VecPursuitController::dynamicParametersCallback,
      this, std::placeholders::_1)
    );
}

void VecPursuitController::deactivate()
{
  RCLCPP_INFO(
    logger_,
...
double VecPursuitController::getLookAheadDistance(
...
double VecPursuitController::calcTurningRadius(
...
geometry_msgs::msg::TwistStamped VecPursuitController::computeVelocityCommands(
  ...
  nav2_core::GoalChecker * goal_checker)
{
    ...
    isCollisionImminent(pose, linear_vel, angular_vel, target_dist))
  {
    linear_vel = 0.0;
    angular_vel = 0.0;
    throw nav2_core::PlannerException("VecPursuitController: Collision detected ahead!");
  }
...
double VecPursuitController::costAtPose(const double & x, const double & y)
{
  unsigned int mx, my;
  ...
  if (!costmap_->worldToMap(x, y, mx, my)) {
    ...
    throw nav2_core::PlannerException(
            "VecPursuitController: Dimensions of the costmap are too small "
...
double VecPursuitController::approachVelocityScalingFactor(
...
void VecPursuitController::applyApproachVelocityScaling(
...
void VecPursuitController::applyConstraints(
...
bool VecPursuitController::shouldRotateToPath(
  const geometry_msgs::msg::PoseStamped & target_pose,
...
bool VecPursuitController::shouldRotateToGoalHeading(
  const geometry_msgs::msg::PoseStamped & target_pose)
{
...
geometry_msgs::msg::Point VecPursuitController::circleSegmentIntersection(
...
geometry_msgs::msg::Quaternion VecPursuitController::getOrientation(
  const geometry_msgs::msg::Point & p1,
  const geometry_msgs::msg::Point & p2)
{
...
geometry_msgs::msg::PoseStamped VecPursuitController::getLookAheadPoint(
...
void VecPursuitController::setPlan(const nav_msgs::msg::Path & path)
{
  global_plan_ = path;
}

void VecPursuitController::rotateToHeading(
...
bool VecPursuitController::isCollisionImminent(
...
double VecPursuitController::getCuspDist(
  const nav_msgs::msg::Path & transformed_plan)
{
...
bool VecPursuitController::inCollision(
  const double & x,
  const double & y,
  const double & theta)
{
...
void VecPursuitController::setSpeedLimit(
...
nav_msgs::msg::Path VecPursuitController::transformGlobalPlan(
  const geometry_msgs::msg::PoseStamped & pose)
{
...
bool VecPursuitController::transformPose(
...
double VecPursuitController::getCostmapMaxExtent() const
{
...
rcl_interfaces::msg::SetParametersResult VecPursuitController::dynamicParametersCallback(
...
// Register this controller as a vec_pursuit_controller
